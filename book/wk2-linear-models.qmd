...
provenance: "Everything printed below came from running this chapter's code from top to bottom when the site was built; nothing is typed from memory. The two errors are real errors, left in on purpose. The R output in the grey boxes was run once by hand, in R, on 2026-09-06, on the same birds, and pasted in."
...
footer_note: "Stats Hours with Itchy · Class 2 of a planned 13 (ten in version 1, plus a coda) · draft · code last run 2026-09-12"
---
...
The `mu:` rows are the μ table above; `sigma:(Intercept)` is the `(Intercept)` row of the
σ table, on the log scale; the `sigma (constant)` line at the bottom is **Residual SD (response
scale)** from the header.
...
Rows correspond as before: `mu:` to the μ table, `sigma:(Intercept)` to the σ table's intercept
on the log scale, `sigma (constant)` to the header's residual SD.
