...
- **`:Year => (x -> length(unique(x))) => :nyear`.** Class 7's anonymous function inside `combine`, here counting distinct values per group.
...
- `drm(bf(@formula(y ~ x + (1|g))), Binomial(); data = df)`: the whole of a GLMM. The family is Class 3's and the grouping term is Class 6's.
...
- `residuals(fit; type = :quantile, rng = MersenneTwister(seed))`: as in Class 3, seeded; check which probability it judges each observation against before reading the spread.
